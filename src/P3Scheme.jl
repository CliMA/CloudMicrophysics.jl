...
look-up table which serves to avoid the longer run time
...
which can be indexed at each time step to generate
...
lie between lookup table points, a linear interpolation between table points
