...
"""
function generate_threshold_table(ρ_r_axis::AbstractRange{FT} = range(start = 100, stop = 950, length = 10), F_r_axis::AbstractRange{FT} = range(start = 0.01, stop = 0.99, length = 10)) where {FT <: Real}
...
lookup_threshold(ρ_r, F_r, vals; opt)
 ...
 - F_r: rime mass fraction (q_rim/q_i, --)
 - vals: tuple of matrices containing lookup values
...
"""
function lookup_threshold(ρ_r::FT, F_r::FT, vals::NTuple{4, Matrix{FT}}, ρ_r_axis::AbstractRange{FT} = range(start = 100, stop = 950, length = 10), F_r_axis::AbstractRange{FT} = range(start = 0.01, stop = 0.99, length = 10)) where {FT <: Real}
    ρ_i = indexin(ρ_r, ρ_r_axis)[1]
    ρ_r_vals = collect(ρ_r_axis)
    println(ρ_r_vals)
    F_i = indexin(F_r, F_r_axis)[1]
    F_r_vals = collect(F_r_axis)
    if typeof(ρ_i) !== Nothing && typeof(F_i) !== Nothing
        lookup = []
        for quantity in vals
            push!(lookup, quantity[ρ_i, F_i])
        end
        return lookup
    else
        function get_left_right(table_axis_vals::Vector, lookup_val::FT)
            ...
            return table_axis_vals[i - 1], table_axis_vals[i]
        end

        if ρ_i === nothing
            ρ_r_left, ρ_r_right = get_left_right(ρ_r_vals, ρ_r)

            return ρ_r_left, ρ_r_right

        elseif F_i === nothing
            F_r_left, F_r_right = get_left_right(F_r_vals, F_r)

            return F_r_left, F_r_right
        end
    end

            # binary search for ρ_r
        # update value to center of half of the remaining portion of the range
        # elseif ρ_r > value
        # do the same

        # now we have value immediately below ρ_r in range
        # subsequent value must be immediately above ρ_r
    
    # add errors that are thrown if ρ_r and F_r are out of bounds of their respective ranges
end
