...
"""
function α_va(FT::Type)
    toml_dict = CP.create_toml_dict(FT; dict_type = "alias") # create CLIMAParameters toml dict object
    α_va_BF1995_pair, β_va_pair =
        CP.get_parameter_values(toml_dict, ["α_va_BF1995", "β_va_BF1995"])[1:2] # grab pairs generated by call to CLIMAParameters
    α_va_BF1995 = α_va_BF1995_pair.second # grab values from pairs
    β_va = β_va_pair.second
    α_va = (α_va_BF1995) * 10^((6 * β_va) - 3) # compute α_va
    return α_va
end
...
"""
function D_th(FT::Type)
    toml_dict = CP.create_toml_dict(FT; dict_type = "alias") # create CLIMAParameters toml dict object
    β_va_pair, ρ_i_pair =
        CP.get_parameter_values(toml_dict, ["β_va_BF1995", "ρ_cloud_ice"])[1:2] # grab pairs generated by call to CLIMAParameters
    # grab values from pairs
    β_va = β_va_pair.second
    ρ_i = ρ_i_pair.second
    # return D_th, making a call to α_va()
    return ((FT(π) * ρ_i) / (6 * α_va(FT)))^(1 / (β_va - 3))
end
