...
function bulk_collision_rate_with_liquid(
    ...
    E = FT(1)  # TODO - Make it a function of Dᵢ and Dₗ
    rᵢ_eff(Dᵢ) = √(ice_area(state, Dᵢ) / π)
    K(Dᵢ, Dₗ) = π * (rᵢ_eff(Dᵢ) + Dₗ / 2)^2  # collision cross section  -- TODO: Check if this is correct
    vᵢ(Dᵢ) = p3_particle_terminal_velocity(state, Dᵢ, vel, ρₐ)
    vₗ(Dₗ) = terminal_velocity(Dₗ, vel.rain, ρₐ)
    Nᵢ(Dᵢ) = N′ice(ice_dist, Dᵢ)
    Nₗ_pdf(Dₗ) = CM2.size_distribution(liquid_dist, Dₗ, Lₗ / ρₐ, ρₐ, Nₗ)

    integrand(Dᵢ, Dₗ) = E * K(Dᵢ, Dₗ) * abs(vᵢ(Dᵢ) - vₗ(Dₗ)) * Nᵢ(Dᵢ) * Nₗ_pdf(Dₗ)
