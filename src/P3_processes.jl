...
"""
    get_liquid_integrals(n, ∂ₜV, m_l, ρᵣ′, liq_bounds, segbuf_liq)
...
- `m_l`: liquid particle mass function m_l(D)
- `ρᵣ′`: local rime density function ρᵣ′(Dᵢ, D)
- `liq_bounds`: integration bounds for liquid particles
- `segbuf_liq`: integration buffer for liquid particles
...
"""
function get_liquid_integrals(n, ∂ₜV, m_l, ρᵣ′, liq_bounds, segbuf_liq = nothing)
    function liquid_integrals(Dᵢ)
        ((∂ₜN_col, ∂ₜM_col, ∂ₜB_col), _) =
            QGK.quadgk(liq_bounds...; eval_segbuf = segbuf_liq) do D
                return SA.SVector(
                    # ∂ₜN_col = ∫ ∂ₜV ⋅ n ⋅ dD
                    ∂ₜV(Dᵢ, D) * n(D),
                    # ∂ₜM_col = ∫ ∂ₜV ⋅ n ⋅ m_l ⋅ dD
                    ∂ₜV(Dᵢ, D) * n(D) * m_l(D),
                    # ∂ₜB_col = ∫ ∂ₜV ⋅ n ⋅ m_l / ρᵣ′ ⋅ dD 
                    ∂ₜV(Dᵢ, D) * n(D) * m_l(D) / ρᵣ′(Dᵢ, D),
                    )
                end
...
"""
    ∫liquid_ice_collisions(
        n_i, ∂ₜM_max, cloud_integrals, rain_integrals,
        ice_bounds, segbuf_ice = nothing,
    )
...
- `ice_bounds`: integration bounds for ice particles
- `segbuf_ice`: integration buffer for ice particles (optional)
...
function ∫liquid_ice_collisions(
    ...
    (rates, _) = QGK.quadgk(liquid_ice_collisions_integrands, ice_bounds...; eval_segbuf = segbuf_ice)
    return rates
end
...
"""
    ∫liquid_ice_collisions(
        ...
        aps, tps, vel, ρₐ, T, m_l
    )

High-level interface for computing liquid-ice collision rates.

# Arguments
- `state`: a [`P3State`](@ref)
- `logλ`: the log of the slope parameter [log(1/m)]
- `psd_c`: a [`CloudParticlePDF_SB2006`](@ref)
- `psd_r`: a [`RainParticlePDF_SB2006`](@ref)
...
- `aps`: air properties
- `tps`: thermodynamics parameters
- `vel`: a [`Chen2022VelType`](@ref)
- `ρₐ`: air density [kg/m³]
- `T`: temperature [K]
- `m_l`: liquid particle mass function m_l(D)

# Returns
A tuple of `(QCCOL, NCCOL, QRCOL, NRCOL, ∫M_col, BCCOL, BRCOL, ∫𝟙_wet_M_col)`, where:
...
function ∫liquid_ice_collisions(
    ...
    ρᵣ′ = compute_local_rime_density(state, vel, ρₐ, T)  # ρᵣ′(Dᵢ, Dₗ)
    ∂ₜM_max = compute_max_freeze_rate(state, aps, tps, vel, ρₐ, T)  # ∂ₜM_max(Dᵢ)

    cloud_integrals = get_liquid_integrals(n_c, ∂ₜV, m_l, ρᵣ′, bounds_c, segbuf_c)  # (∂ₜN_c_col, ∂ₜM_c_col, ∂ₜB_c_col)
    rain_integrals = get_liquid_integrals(n_r, ∂ₜV, m_l, ρᵣ′, bounds_r, segbuf_r)  # (∂ₜN_r_col, ∂ₜM_r_col, ∂ₜB_r_col)

    return ∫liquid_ice_collisions(
        n_i, ∂ₜM_max, cloud_integrals, rain_integrals, ice_bounds, segbuf_ice,
    )
end
