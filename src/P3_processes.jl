...
function bulk_collision_rate_with_liquid(
    ...
    segbuf_ice = QGK.quadgk_segbuf(N′_ice_psd, bounds...)[3]
    ...
    function ice_integrand(D_ice)
        ...
        # Resulting rates
        ∂ₜL_rim_source_integrand = ∂ₜmass_freezing * N′_ice               # [kg m⁻³ / s / m]
        ∂ₜN_rain_source_integrand = ∂ₜmass_shedding / shed_mass * N′_ice  # [   m⁻³ / s / m]
        ∂ₜL_rain_source_integrand = ∂ₜmass_shedding * N′_ice              # [kg m⁻³ / s / m]
        ∂ₜB_rim_source_integrand = ∂ₜmass_freezing / ρᵣ′

        # Integrating over `D_ice` gives another unit of `[m]`, so `[X / s / m]` --> `[X / s]`
        return (
            ∂ₜq_cloud_part * N′_ice,    # ∂ₜq_cloud sink (QCCOL)
            ∂ₜN_cloud_part * N′_ice,    # ∂ₜN_cloud sink (NCCOL)
            ∂ₜq_rain_part * N′_ice,     # ∂ₜq_rain sink (QRCOL)
            ∂ₜN_rain_part * N′_ice,     # ∂ₜN_rain sink (NRCOL)
