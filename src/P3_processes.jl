...
function ice_melt(
    ...
    state = get_state(params; F_rim, F_liq, ρ_r)
    dist = distribution_parameter_solver(state, log(L_ice), log(N_ice))
    N(D) = N′ice(dist, D)
    bound = FT(1)
    ...
    # Ventillation factor
    vent = params.vent
    F_v(D) = vent.vent_a + vent.vent_b * N_sc^(1 / 3) * N_Re(D)^(1 / 2)
    dmdD(D) = ∂ice_mass_∂D(state, D)

    f(D) = 4 * K_therm / L_f * (Tₐ - params.T_freeze) * dmdD(D) / D * F_v(D) * N(D)
    # Integrate
    (dLdt, error) = QGK.quadgk(f, 0, thresholds..., bound, rtol = FT(1e-6))
