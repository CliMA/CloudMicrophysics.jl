...
accKK2000 = [CM2.accretion_KK2000(param_set, q_liq, 5.0e-4, 1.0) for q_liq in q_liq_range]
accB1994 = [CM2.accretion_B1994(param_set, q_liq, 5.0e-4) for q_liq in q_liq_range]
accTC1980 = [CM2.accretion_TC1980(param_set, q_liq, 5.0e-4) for q_liq in q_liq_range]

fig = Figure()
ax1 = Axis(fig[1, 1]; yscale = log10)
ax2 = Axis(fig[2, 1])
ylims!(ax1, [1e-13, 1e-5])
ylims!(ax2, [1e-13, 5e-6])
...
l10 = lines!(ax2, q_liq_range * 1e3, accKK2000, color = :red)
l11 = lines!(ax2, q_liq_range * 1e3, accB1994, color = :green)
l12 = lines!(ax2, q_liq_range * 1e3, accTC1980, color = :blue)
ax1.xlabel = "q_liq [g/kg]"
ax1.ylabel = "autoconversion rate [kg m^-3 s^-1]"
ax2.xlabel = "q_liq [g/kg] (q_D = 0.5e-4)"
ax2.ylabel = "accretion rate [?]"
Legend(fig[1, 2], 
