...
"""
p3_logλ_1d(::Type{FT}) where {FT} = p3_logλ(FT, make_column(FT))
...
bmt_2mp3_1d(::Type{FT}, mode) where {FT} = bmt_2mp3(FT, make_column(FT), mode)

bmt_2mp3_3d(::Type{FT}, mode) where {FT} = bmt_2mp3(FT, get_rcemipii_center_space(FT), mode)

function bmt_2mp3(::Type{FT}, space, mode) where {FT}
...
TT.@testset "ClimaCore GPU inference failure $nD $FT" for nD in ("1D", "3D"), FT in (Float64, Float32)
    ...
    TT.@testset "bulk microphysics tendencies (2M+P3 Rosenbrock)" begin
        ...
        bmt_2mp3_nd(FT, BMT.rosenbrock_manual())
        # The exact Jacobian allocates about 120 KB per call on the CPU, which the GPU
        # compiler rejects as a GC allocation. Broken until that path is allocation-free.
        exact_compiles = try
            bmt_2mp3_nd(FT, BMT.rosenbrock_exact())
            true
        catch
            false
        end
