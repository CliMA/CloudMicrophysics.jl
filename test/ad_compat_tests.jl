...
function test_ad_compatibility(FT)
    ...
    @testset "BMT 2M+P3 Jacobian w.r.t. the 8 species ($FT)" begin
        ...
        # Known boundary: SB2006 autoconversion Φ_au(τ) ∝ τ^0.7 is not
        # differentiable at exactly zero rain with cloud present. The primal is
        # finite; the warm-rain rows of the Jacobian are NaN. Pinned here so a
        # future smoothing of Φ_au shows up as a test change.
        ...
        @test any(isnan, FD.jacobian(f_b, x_boundary))
    end
end
