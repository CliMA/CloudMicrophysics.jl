...
TT.@testset "Nucleation Unit Tests" begin
    FT = Float64
    ...
    TT.@testset "Pure H2SO4 Nucleation Smoke Test" begin
        nh3_conc = 0.0
        ...
        for (h2so4_conc, rate) in zip(h2so4_concentrations, rates)
            TT.@test test_h2so4(h2so4_conc) ≈ rate rtol = 1e-5
        end
    end


    TT.@testset "Pure Organic Nucleation Smoke Test" begin
        negative_ion_conc = 0.0
        test_organic(hom_c) =
            sum(
        ...
        for (hom_conc, rate) in zip(hom_concentrations, rates)
            TT.@test test_organic(hom_conc) ≈ rate rtol = 1e-5
        end
    end

    TT.@testset "Mixed Organic and H2SO4 Nucleation Smoke Test" begin
        h2so4_conc = 2.6e6
        ...
        for (bioOxOrg_conc, rate) in zip(bioOxOrg_concentrations, rates)
            TT.@test test_mixed_nucleation(bioOxOrg_conc) ≈ rate rtol = 1e-5
        end
    end
end # testset
nothing
