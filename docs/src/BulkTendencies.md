...
Because the forward-Euler step has no implicit damping of its own, `SubsteppedAverage` carries a per-substep positivity floor and the increment limiter to bound the otherwise-unstable increments:

- A per-substep positivity floor clamps every species to nonnegative values after each substep (``\max(x, 0)``), the same floor used by the linearized-implicit modes. In the two-moment scheme it floors each paired mass and number species, so a number content that reaches zero takes its mass to zero on the same substep.
- When the limiter is `EndStateSaturationAdjustment`, the saturation-adjustment limiter scales the whole increment so a single explicit step does not overshoot saturation over the more-supersaturated phase. With `NoLimiter` this limiter is omitted, which is appropriate when the increment is taken inside an outer implicit timestepper that supplies the stability.
...
For the substepped average to converge to the pointwise trajectory as ``h \to 0``, its limiter must be *consistent*: the correction it applies must vanish as the substep shrinks, so that it changes only steps that would otherwise cross saturation and leaves the converged solution unbiased. Every averaging mode — `SubsteppedAverage` and both linearized-implicit averages, in the one-moment and the two-moment + P3 schemes — uses the same consistent limiter: a bisection that scales the whole increment only when the full latent-heated end state would cross saturation over its more-supersaturated phase, and is inactive otherwise (see [Increment scaling](@ref)). It vanishes as ``h \to 0``, and the accompanying per-substep positivity floor (``\max(x, 0)``) likewise vanishes under refinement, so all modes converge in the warm-rain, cold/ice, and mixed-phase regimes.

The modes differ not in their limiter but in how many substeps they need. The linearized-implicit average damps the stiff decay and vapor-exchange modes, so it is accurate at few substeps even in stiff regimes. The explicit `SubsteppedAverage` is forward Euler, with no implicit damping, so it needs ``h`` small relative to the fastest process time scale; in the stiff mixed-phase regime it converges to the same saturation-adjusted trajectory but requires many more substeps to reach the same accuracy.

The two families are therefore appropriate in different settings. The linearized-implicit average is preferred when stability or accuracy at coarse substep counts is the priority and the cost of forming and solving the substep system is acceptable; it is the operational one-moment choice. The explicit average is cheaper per substep — no Jacobian, no linear solve — but needs more substeps in stiff regimes; for the two-moment + P3 scheme the linearized-implicit `rosenbrock_exact` (`ExactJacobian` with `ExplicitGrowthDiagonal` and `EndStateSaturationAdjustment`) is roughly an order of magnitude more expensive per microphysics evaluation than the explicit `SubsteppedAverage`; amortized against the rest of a host-model step this difference is typically much smaller, so at the full-step level the cost difference is often modest. The numerics that motivate `rosenbrock_exact` — the coarse-step deposition instability and the mixed-phase saturation criterion — are detailed in [Bulk-tendency averaging: modes and numerics](@ref).
