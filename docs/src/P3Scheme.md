...
!!! note
    The original paper [MorrisonMilbrandt2015](@cite) uses symbol ``q`` to denote the mass of a tracer per volume of air 
...
!!! details "Symbol definitions"
    ...
    |:----------|:----------------------------------------|--------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | $D_{th}$  |                                         | m                  | small, spherical ice threshold                                                                                                                                     |
    | $D_{gr}$  |                                         | m                  | graupel threshold                                                                                                                                                  |
    | $D_{cr}$  |                                         | m                  | partially rimed ice threshold                                                                                                                                      |
    | $ρ_i$     | 916.7                                   | kg$~$m$^{-3}$      | cloud ice density                                                                                                                                                  |
    | $ρ_g$     |                                         | kg$~$m$^{-3}$      | bulk density of graupel                                                                                                                                            |
    | $ρ_r$     |                                         | kg$~$m$^{-3}$      | rime density                                                                                                                                                       |
    | $ρ_d$     |                                         | kg$~$m$^{-3}$      | bulk density of unrimed ice                                                                                                                                        |
    ...
    | $α_{va}$  | $7.38 \; 10^{-11} \; 10^{6 β_{va} - 3}$ | kg$~$m$^{-β_{va}}$ | parameter modified for units from [BrownFrancis1995](@cite) in base SI units (also based on measurements of vapor diffusion and aggregation in midlatitude cirrus) |
    | $γ$       | 0.2285                                  | m$^{2-σ}$          | parameter fit from aggregates of side planes, columns, bullets, and planar polycrystals; see Table 1 in [Mitchell1996](@cite)                                      |
...
```
Given $ρ_d$, we can obtain $ρ_g$, $D_{gr}$, and $D_{cr}$ using the expressions above. Depending on the value of $ρ_r$ and $F_{rim}$, these thresholds and densities obtain a range of values, as shown in the plot below.
...
To close the system of equations, it has been customary (refs) to assume $μ$ to be a function of $λ$, $μ = μ(λ)$.
...
rewriting the first equation. For numerical stability, we compute $\log(L_\mathrm{ice}/N_\mathrm{ice})$ 
...
!!! details "Computing Distribution Integrals Using Gamma Functions"
    The integrals in our microphysics scheme can be evaluated analytically using the Gamma function family. Here we present the key formulas and their numerical implementation.
    ##### Core Definitions
...
In [MorrisonMilbrandt2015](@cite), the following coefficients and limiting behavior are used:
