...
terminal velocity(radius), denoted by ``v_\text{term}(r)``, respectively.
...
|``m_0^{rai}``  | coefficient in ``m(r)`` for rain        | ``kg``          | ``\frac{4}{3} \, \pi \, \rho_\text{water} \, r_0^3`` |           |
...
|    symbol                  |         definition                                          | units                    | default value          | reference                        |
|----------------------------|-------------------------------------------------------------|--------------------------|------------------------|----------------------------------|
|``\tau_\text{acnv_rain}``   | cloud liquid to rain water autoconversion timescale         | ``s``                    | ``10^3``               | eq (5a) [Grabowski1996](@cite)   |
...
```math
\begin{equation}
  F(r) = a_\text{vent} + b_\text{vent}  N_{Sc}^{1/3} N_{Re}(r)^{1/2}
...
```math
\begin{equation}
\left. \frac{d \, q_{rai}}{dt} \right|_\text{accr_ri} =
...
```math
\begin{equation}
  \left. \frac{dq}{dt} \right|_\text{evap_subl} =
