...
This guide describes the development workflow, repository organization,
  and common pitfalls associated with contributing to `CloudMicrophysics.jl`.
...
- The `ci` runs the tests on Ubuntu, Windows and macOS machines using GitHub Actions.
  ...
  See the [Tests](@ref) section below for debugging tips.
...
- The `Documentation / docbuild` builds the documentation.
  ...
  see the [Documentation](@ref) section for hints.
...
Each new addition to the library should be accompanied by a documentation page
  summarizing the derivation/assumptions and its potential uses.
If possible, also add short code snippets that reproduce
  results from the literature.
Those code snippets are executed every time the documentation is built and provide
  useful examples of how the available parameterizations work.
In some cases, it may be useful to add simple usage examples to the docstrings.
  This is achieved by using the `jldoctest` environment described in the
...
The docstring formatting is strict:
...
Exported functions with missing docstrings, or docstrings not included in the
  API page, cause the documentation build to break
  (the build is checked with `checkdocs = :exports`).
...
Performance tests check the memory allocations (typically there should be none)
  and execution times of some of the functions.
...
Next, add your performance tests under the comment with the file name which your
  new function is in.
This is done by calling `bench_press(FT, function_name, (Parameter1, Parameter2), min_time)`,
  where the first argument is the expected return type of the function.
...
You can identify which specific performance test is failing on GitHub
  by clicking on details next to a failed `ci` check.
...
Kernels are added at the top half of the file using
  `@kernel inbounds = true`.
The naming convention typically follows `test_<function name>_kernel!`.
Within the kernel, place any outputs into an output array.
...
For example, if you want to test at temperature of 230 K,
  you can add `T = ArrayType([FT(230)])`.
Add a comment of what you are testing and use `@test` to create your test.
The GPU tests are run twice: for `Float64` and `Float32`.
As with the performance tests, some trial and error is needed
  to find good tolerances for both options.

### P3 quadrature error study

The P3 scheme evaluates its size-distribution integrals with a Gauss-Legendre rule whose default
order is recorded in `src/parameters/Microphysics2MParams.jl`. The study that supports the default
lives in `test/p3_quadrature_error_study.jl`: it measures the relative error of every
quadrature-consuming P3 quantity against a high-order reference, across a set of physically
plausible column states, and attributes it to the transport components (sedimentation velocities,
melt), the components proportional to the collision efficiency, and the full bulk tendency vector.
...
Rerun the study when changing the quadrature rule, the integral bounds or breakpoints, or the P3
process integrands, and revisit the default order and the tolerances in
`test/bulk_tendencies_quadrature_tests.jl` accordingly.
