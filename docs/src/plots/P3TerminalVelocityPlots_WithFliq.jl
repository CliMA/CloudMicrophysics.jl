...
function figure_2()
    ...
    crange_small(len) = range(0.39, 0.6, length = len)
    ...
    ax4 = make_axis(fig, 3, 1, "Vₘ with small Dₘ, F_liq = 0.33")
    hm = Plt.contourf!(ax4, F_rims_33, ρ_rs_33, V_ms_33, levels = crange_small(20), extendlow = :auto, extendhigh = :auto)
    Plt.contour!(ax4, F_rims_33, ρ_rs_33, D_ms_33; args...)
    # Plt.Colorbar(fig[4, 1], hm, vertical = false)

    ax5 = make_axis(fig, 3, 2, "Vₘ with medium Dₘ, F_liq = 0.33")
    ...
    ax6 = make_axis(fig, 3, 3, "Vₘ with large Dₘ, F_liq = 0.33")
    ...
    ax7 = make_axis(fig, 5, 1, "Vₘ with small Dₘ, F_liq = 0.67")
    hm = Plt.contourf!(ax7, F_rims_67, ρ_rs_67, V_ms_67, levels = crange_small(30), extendlow = :auto, extendhigh = :auto)
    Plt.contour!(ax7, F_rims_67, ρ_rs_67, D_ms_67; args...)
    Plt.Colorbar(fig[6, 1], hm, vertical = false)

    ax8 = make_axis(fig, 5, 2, "Vₘ with medium Dₘ, F_liq = 0.67")
    ...
    ax9 = make_axis(fig, 5, 3, "Vₘ with large Dₘ, F_liq = 0.67")
