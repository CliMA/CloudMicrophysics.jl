...
function aspect_ratio_snow_1M(snow::CMP.Snow, D::FT) where {FT <: Real}
...
function snow_terminal_velocity_individual_Chen(
    ...
    #TODO - will be fixed in a next PR
    ϕ = aspect_ratio_snow_1M(snow, D)

    v = 0
    for i in 1:2
        v += ai[i] * D^bi[i] * exp(-D * ci[i])
    end
    v_term = ifelse(ϕ > 0, ϕ^(-1 / 3) * v, ϕ^(1 / 6) * v)
...
# aspect ratio plot
Aspect_Ratio = [aspect_ratio_snow_1M(snow, D_r) for  D_r in D_r_range]
...
# snow aspect ratio
p3 = PL.plot(D_r_range * 1e3,  Aspect_Ratio, linewidth = 3, xlabel = "D [mm]", ylabel = "aspect ratio", label = "Snow aspect ratio", color = :darkviolet)
