...
function make_slope_plot(slope_law, title)
    fig = MK.Figure(size = (400, 300), figure_padding = 20)

    ax = MK.Axis(fig[1, 1]; title, xscale = log10, xlabel = "λ", ylabel = "μ")
    MK.vlines!(λ_bnds, color = (:gray, 0.5))

    MK.lines!(ax, exp.(logλs), P3.get_μ.(slope_law, logλs))
...
fig = MK.with_theme(MK.theme_minimal()) do
    make_slope_plot(slope_power_law, "μ as a function of λ (power law)")
end
MK.save("P3SlopeParameterizations_power_law.svg", fig)
...
function make_multiple_solutions_plot()
    ...
    # Make figure
    fig = MK.Figure(size = (800, 300), figure_padding = 20)
    ax = MK.Axis(fig[1, 1];
        title = "Valid solutions of shape solver",
        xscale = log10, xlabel = "λ", ylabel = "shape solution", limits = ((10^3.55, 10^4.7), (-0.5, 0.5)),
    )
    MK.vlines!(λ_bnds, color = (:gray, 0.5))
    MK.hlines!(ax, 0; color = (:gray, 0.2))

    MK.lines!(ax, exp.(logλs), shape_sol; color = :black)
    map(dists) do dist
        λ = exp(dist.log_λ)
        MK.vlines!(ax, λ; label = Printf.@sprintf("λ = %.0f m⁻¹", λ))
    end
    ...
    MK.vlines!(state.D_th * m_to_mm, color = (:gray, 0.5))
    for dist in dists
        N′ = @. exp(P3.log_N′ice(dist, Ds))
        MK.lines!(Ds_mm, N′ / m_to_cm^4)
        # MK.lines!(Ds_mm, cumsum(N′.(Ds) .* Ds / m_to_cm^3))  # CUMULATIVE DISTRIBUTION
    end
...
MK.save("P3SlopeParameterizations_multiple_solutions.svg", fig)
