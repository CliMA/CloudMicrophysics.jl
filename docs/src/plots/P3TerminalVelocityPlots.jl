...
function get_values(
	...
	for i in 1:x_resolution
		for j in 1:y_resolution
			...
			state = P3.get_state(params; F_rim, F_liq, ρ_r)
			dist = P3.distribution_parameter_solver(state, log(L), log(N))

			try
				V_m[i, j] = P3.ice_terminal_velocity(dist, Chen2022, ρ_a, do_not_use_aspect)[2]
			catch e
				@show F_rim, ρ_r, L, N, ρ_a, do_not_use_aspect
				rethrow()
			end
			V_m_ϕ[i, j] = P3.ice_terminal_velocity(dist, Chen2022, ρ_a, use_aspect_ratio)[2]

			D_m[i, j] = P3.D_m(state, log(L), log(N))
...
function figure_2()
	...
	hm = contourf!(
		...
		D_m_regimes_s,
		levels = 3,
		colormap = cgrad(:PuBuGn_3, 3, categorical = true),
	)
	Colorbar(
		fig[2, 1],
		hm,
	...
	hm = contourf!(ax4, F_rims, ρ_rs, D_m_s)
	Colorbar(fig[4, 1], hm, vertical = false)

	ax5 = make_axis(fig, 3, 2, "Dₘ (mm) (L = 0.22 kgm⁻³, N = 1e6 m⁻³)")
	hm = contourf!(ax5, F_rimm, ρ_rm, D_m_m)
	Colorbar(fig[4, 2], hm, vertical = false)

	ax6 = make_axis(fig, 3, 3, "Dₘ (mm) (L = 0.7 kgm⁻³, N = 1e6 m⁻³)")
	hm = contourf!(ax6, F_riml, ρ_rl, D_m_l)
	Colorbar(fig[4, 3], hm, vertical = false)

	ax7 = make_axis(fig, 5, 1, "ϕᵢ with small Dₘ")
	hm = contourf!(ax7, F_rims, ρ_rs, ϕᵢ_s, levels = 20)
	Colorbar(fig[6, 1], hm, vertical = false)
	contour!(ax7, F_rims, ρ_rs, D_m_s, label = "D_m"; contour_args...)

	ax8 = make_axis(fig, 5, 2, "ϕᵢ with medium Dₘ")
	hm = contourf!(ax8, F_rimm, ρ_rm, ϕᵢ_m)
	Colorbar(fig[6, 2], hm, vertical = false)
	contour!(ax8, F_rimm, ρ_rm, D_m_m, label = "D_m"; contour_args...)

	ax9 = make_axis(fig, 5, 3, "ϕᵢ with large Dₘ")
	hm = contourf!(ax9, F_riml, ρ_rl, ϕᵢ_l)
	Colorbar(fig[6, 3], hm, vertical = false)
	contour!(ax9, F_riml, ρ_rl, D_m_l, label = "D_m"; contour_args...)

	ax10 = make_axis(fig, 7, 1, "Vₘ (ϕᵢ = 1) with small Dₘ")
	hm = contourf!(ax10, F_rims, ρ_rs, V_m_s)
	contour!(ax10, F_rims, ρ_rs, D_m_s; contour_args...)
	Colorbar(fig[8, 1], hm, vertical = false)

	ax11 = make_axis(fig, 7, 2, "Vₘ (ϕᵢ = 1) with medium Dₘ")
	hm = contourf!(ax11, F_rimm, ρ_rm, V_m_m)
	contour!(ax11, F_rimm, ρ_rm, D_m_m; contour_args...)
	Colorbar(fig[8, 2], hm, vertical = false)

	ax12 = make_axis(fig, 7, 3, "Vₘ  (ϕᵢ = 1) with large Dₘ")
	hm = contourf!(ax12, F_riml, ρ_rl, V_m_l)
	contour!(ax12, F_riml, ρ_rl, D_m_l; contour_args...)
	Colorbar(fig[8, 3], hm, vertical = false)

	ax13 = make_axis(fig, 9, 1, "Vₘ (using ϕᵢ) with small Dₘ")
	hm = contourf!(ax13, F_rims, ρ_rs, V_m_ϕ_s)
	contour!(ax13, F_rims, ρ_rs, D_m_s; contour_args...)
	Colorbar(fig[10, 1], hm, vertical = false)

	ax14 = make_axis(fig, 9, 2, "Vₘ (using ϕᵢ) with medium Dₘ")
	hm = contourf!(ax14, F_rimm, ρ_rm, V_m_ϕ_m)
	contour!(ax14, F_rimm, ρ_rm, D_m_m; contour_args...)
	Colorbar(fig[10, 2], hm, vertical = false)

	ax15 = make_axis(fig, 9, 3, "Vₘ  (using ϕᵢ) with large Dₘ")
	hm = contourf!(ax15, F_riml, ρ_rl, V_m_ϕ_l)
	contour!(ax15, F_riml, ρ_rl, D_m_l; contour_args...)
	Colorbar(fig[10, 3], hm, vertical = false)

	linkaxes!(ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9, ax10, ax11, ax12, ax13, ax14, ax15)

	resize_to_layout!(fig)
	save("MorrisonandMilbrandtFig2.svg", fig)
	fig
end
#! format: on
