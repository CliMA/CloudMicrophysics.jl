...
function generate_cond_evap_rate(::HydrostaticBalance_qₗ_z)
    ...
    result = @. CMNe.conv_q_vap_to_q_lcl_icl_MM2015(cm_params, thp, qₜ, qₗ, qᵢ, qᵣ, qₛ, ρ, T)
    data = first.(result)
...
function make_deriv_line_plot()
    ...
    for (i, T) in enumerate(T_range)
        T_ft = FT(T)
        (s, ds_s, ds_f) = CMNe.conv_q_vap_to_q_lcl_icl_MM2015(
...
function make_deriv_vs_qvap_plot()
    ...
    for (j, T) in enumerate(T_vals)
        ...
        for (i, qₜ) in enumerate(qₜ_range)
            ...
            (s, _, ds_f) = CMNe.conv_q_vap_to_q_lcl_icl_MM2015(
