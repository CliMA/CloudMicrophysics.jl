...
qᵢ = FT(0) #mi_v / (md_v + mv_v + ml_v + mi_v)
...
ax7 = MK.Axis(fig[4, 1], xlabel = "Temperature [K]", ylabel = "SLF")
...
for val in dt_vals
    ...
    for (i, CD) in enumerate(condensation_growth_list)
        ...
        # Plot results
        MK.lines!(ax1, sol.t, (sol[1, :] .- 1) * 100.0, color = color, alpha = val * 10, label = string(val))
        MK.lines!(ax2, sol.t, sol[3, :], color = color, alpha = val * 10)
        MK.lines!(ax3, sol.t, sol[5, :] * 1e3, color = color, alpha = val * 10)
        MK.lines!(ax4, sol.t, (S_i.(tps, sol[3, :], sol[1, :]) .- 1) * 100.0, color = color, alpha = val * 10)
        MK.lines!(ax5, sol.t, sol[4, :] * 1e3, color = color, alpha = val * 10)
        MK.lines!(ax6, sol.t, sol[6, :] * 1e3, color = color, alpha = val * 10)
        ...
        MK.lines!(ax7, sol_T, sol[5, :] ./ (sol[6, :] .+ sol[5, :]), color = color, alpha = val * 10)
    ...
    #=     diff_Sl = saved_sol[2][1, :] - saved_sol[1][1, :]
        diff_Si = (S_i.(tps, saved_sol[2][3, :], saved_sol[2][1, :]) .- 1) - (S_i.(tps, saved_sol[1][3, :], saved_sol[1][1, :]) .- 1)
        ...
        # Plot results
        MK.lines!(ax1, saved_sol[1].t, diff_Sl, label=string(val))
...
MK.save("noneq_difference_dt.svg", fig)
nothing
