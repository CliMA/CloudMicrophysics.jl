...
function parcel_model(dY, Y, p, t)
    ...
    # Our state vector
    S_liq = Y[1]      # supersaturation over liquid water
    p_a = Y[2]        # pressure
    T = Y[3]          # temperature
    ...
    ξ = TD.saturation_vapor_pressure(thermo_params, T, TD.Liquid()) / TD.saturation_vapor_pressure(thermo_params, T, TD.Ice())
    S_i = ξ * S_liq + ξ - 1
    ...
    if "Deposition" in growth_modes && N_ice > 0
        ...
        C_i = r_i
        dqi_dt_depo = 4 * π / ρ_air * S_i * G_i * r_i * N_ice
    ...
    if "Condensation" in growth_modes && N_liq > 0
        ...
        C_l = r_l
        dql_dt_cond = 4 * π / ρ_air * S_liq * G_l * r_l * N_liq
    end
    ...
    dS_l_dt = a1 * w * (S_liq + 1) - (a2 + a3 * (S_liq + 1)) * dql_dt - (a2 + a4 * (S_liq + 1)) * dqi_dt
    dp_a_dt = -p_a * grav / R_a / T * w
    dT_dt = -grav / cp_a * w + L_vap / cp_a * dql_dt + L_subl / cp_a * dqi_dt
    ...
    # Set tendencies
    dY[1] = dS_l_dt        # supersaturation over liquid water
    ...
    dY[10] = dx_sulph_dt   # sulphuric acid concentration
...
S_l - supersaturation over liquid water,
