...
q_icl_to_q_sno_rate = T -> first.(CM1.conv_q_icl_to_q_sno.(ice, aps, tps, q_tot, FT(0), q_icl_range, q_rai, q_sno, ρ_air, T))
MK.lines!(q_lcl_range * 1e3, first.(CM1.conv_q_lcl_to_q_rai.(rain.acnv1M, q_lcl_range)), label = "Rain")
...
MK.lines!(
    q_rain_range * 1e3, first.(CM1.accretion.(liquid, rain, Blk1MVel.rain, ce, q_lcl, q_rain_range, ρ_air)),
    label = "Liq+Rain-CliMA",
)
MK.lines!(
    q_rain_range * 1e3, first.(CM1.accretion.(ice, rain, Blk1MVel.rain, ce, q_icl, q_rain_range, ρ_air)),
    label = "Ice+Rain-CliMA",
)
MK.lines!(
    q_snow_range * 1e3, first.(CM1.accretion.(liquid, snow, Blk1MVel.snow, ce, q_lcl, q_snow_range, ρ_air)),
    label = "Liq+Snow-CliMA",
)
MK.lines!(
    q_snow_range * 1e3, first.(CM1.accretion.(ice, snow, Blk1MVel.snow, ce, q_icl, q_snow_range, ρ_air)),
...
_accr_rain_sink(q_icl) = first.(CM1.accretion_rain_sink.(rain, ice, Blk1MVel.rain, ce, q_icl, q_rain_range, ρ_air)) # hide
...
_accr_snow_rain(q_sno) =
    first.(CM1.accretion_snow_rain.(rain, snow, Blk1MVel.rain, Blk1MVel.snow, ce, q_rain_range, q_sno, ρ_air))
...
_accr_snow_rain2(q_sno) =
    first.(CM1.accretion_snow_rain.(snow, rain, Blk1MVel.snow, Blk1MVel.rain, ce, q_snow_range, q_sno, ρ_air))
...
MK.lines!(
    q_rain_range * 1e3,
    first.(CM1.evaporation_sublimation.(
        rain,
        Blk1MVel.rain,
        aps,
        tps,
...
T = 273.15
_snow_melt(ΔT) = first.(CM1.snow_melt.(snow, Blk1MVel.snow, aps, tps, q_snow_range, ρ, T + ΔT))
...
for q_rai_val in [FT(5e-4), FT(1e-3), FT(2e-3)]
    ...
    for (i, T) in enumerate(T_range)
        ...
        ρ_loc = p / R / T
        (r, dr) = CM1.evaporation_sublimation(
...
for q_sno_val in [FT(5e-5), FT(1e-4), FT(5e-4)]
    ...
    for (i, T) in enumerate(T_range_warm)
        (r, dr) = CM1.snow_melt(snow, Blk1MVel.snow, aps, tps, q_sno_val, ρ_loc, T)
        rates[i] = r
        drates[i] = dr
    end
