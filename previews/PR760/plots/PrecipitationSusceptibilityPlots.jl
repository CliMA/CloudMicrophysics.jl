...
ax.xlabel = "q_rai / (q_lcl + q_rai)"
ax.ylabel = "Precipitation susceptibility"

l1 = lines!(ax, τ, [r.d_ln_pp_d_ln_q_lcl for r in aut_rates], color = :red)
l2 = lines!(ax, τ, [r.d_ln_pp_d_ln_q_rai for r in aut_rates], color = :brown)
l3 = lines!(ax, τ, [r.d_ln_pp_d_ln_q_lcl for r in acc_rates], color = :blue)
